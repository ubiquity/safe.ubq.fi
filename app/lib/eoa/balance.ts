import { Contract, ethers, JsonRpcProvider, JsonRpcSigner, Wallet } from "ethers";
...
export async function getNativeBalance(network: keyof typeof TOKENS, address: `0x${string}`, signer: Wallet | JsonRpcSigner) {
    let provider;
...
function getUrl(method: keyof typeof geckoMethods, args: Record<string, string>) {
    ...
    return url.replace(/{(.*?)}/g, (_, match) => args[match]);
}
